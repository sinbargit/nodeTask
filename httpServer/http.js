...
var stringProcess = require('string');
...
if (process.argv[3]) {
    ...
    basePath = pathProcess.normalize(basePath);
...
var server = http.createServer(function (req, resp) {
    ...
    stringProcess(httpPath).endsWith('/')&&(httpPath = urlObj.pathname.slice(0, -1));
    var path = basePath + httpPath;
            ...
            if (stat.isFile()) {
                ...
                fs.readdir(path, function (err, list) {
                    ...
                    list.forEach(function (item) {
                        aList.push('<div><a href="' + urlProcess.format({
                                protocol: 'http',
                                slashes: true,
                                host: host,
                                pathname: httpPath + '/' + item
                            }) + '">' + item + '</a></div>');
                    });
